...
const ExpenseList = React.memo(({ expenses, budgets }) => {
    ...
    return (
        <div >
            <section className="panel panel-info">
                <header className="panel-heading">
                    <h5 className="panel-title">{t("app.expenseListTitle")}</h5>
                </header>
                <div className="panel-body row">
                        ...
                        <div key={expense.expensesId} className="col-lg-4 col-md-6 mb-4">
                            <div className="card h-100 shadow-sm custom-card-border">
                                <div className="card-body">
                                    ...
                                    <p className="card-text"><strong>{t("app.add-expenses-date")}: </strong>{formatDate(expense.expensesDate)}</p>
                    ...
                    {showDeleteConfirmation && (
                        <BasicModal show={showDeleteConfirmation} handleClose={handleCloseModal}title={t("app.budgetListConfirmDeletion")}>
                            <div className="text-center">
                                <h5>{t("app.expenseListTitleSureDelete")}</h5>
                                <div className="action-buttons mrgn-tp-md">
                                    <button className="btn btn-danger m-2" onClick={confirmDelete}>{t("app.budgetItem-confirm-delete")}</button>
                                    <button className="btn btn-default m-2" onClick={() => setShowDeleteConfirmation(false)}>{t("app.budgetItem-cancel")}</button>
                                </div>
                            </div>
                        </BasicModal>
                    )}
