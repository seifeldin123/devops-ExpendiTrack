...
describe('BudgetService', () => {
    ...
    it('TC_UI_002: should handle duplicate budget name error', async () => {
        ...
        await expect(createBudget({ userId: 1, budgetDescription: "ExistingBudget", budgetAmount: 300 })).rejects.toThrow(errorMessage);
    ...
    it('TC_UI_003: should handle non-existent user error', async () => {
        ...
        await expect(createBudget({ userId: 999, budgetDescription: "NewBudget", budgetAmount: 200 })).rejects.toThrow(errorMessage);
    });
    ...
    it('TC_UI_004: should handle invalid budget amount error', async () => {
        ...
        await expect(createBudget({ userId: 1, budgetDescription: "InvalidBudget", budgetAmount: -100 })).rejects.toThrow(errorMessage);
    });
    ...
    it('TC_UI_008: should reject budget creation due to invalid budget name', async () => {
        ...
        await expect(createBudget({ userId: 1, budgetDescription: "!@#$%", budgetAmount: 200 })).rejects.toThrow(errorMessage);
    });
});
